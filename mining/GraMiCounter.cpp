...
int GraMiCounter::is_frequent(GraphX *graph, Pattern *pattern, int support, double approximate,
    ...
    for (std::tr1::unordered_map<int, NodeX *>::const_iterator iter = pg->get_nodes_iterator(); iter !=
                                                                                           pg->get_nodes_end_iterator(); ++iter) {
        int varNodeID = iter->first;
        domains_values.insert(std::pair<int, std::tr1::unordered_set<int> *>(varNodeID, new std::tr1::unordered_set<int>()));
    }
    ...
    for (int i = 0; i < pattern_size; i++) {
        ...
        int counter = 0;
        for (std::tr1::unordered_set<int>::iterator iter = current_domain->begin(); iter != current_domain->end(); ++iter) {
...
void
GraMiCounter::AC_3(GraphX *graph, std::tr1::unordered_map<int, std::tr1::unordered_set<int> *> &domains_values, Pattern *pattern,
                   int support) {
    GraMiCounter::AC_3(graph, domains_values, pattern->get_graph(), support, pattern->get_invalid_col());
}

void
GraMiCounter::AC_3(GraphX *graph, std::tr1::unordered_map<int, std::tr1::unordered_set<int> *> &domains_values, GraphX *p_graph,
                   int support, int invalid_col) {
    std::map<std::string, pair_with_edge *> arcs;

    //create pairs of connection from the pattern
    for (std::tr1::unordered_map<int, NodeX *>::const_iterator iter = p_graph->get_nodes_iterator(); iter !=
                                                                                                p_graph->get_nodes_end_iterator(); ++iter) {
        NodeX *pNode = iter->second;
        for (std::tr1::unordered_map<int, void *>::iterator iter1 = pNode->get_edges_iterator(); iter1 !=
            ...
            if (arcs.find(sig) != arcs.end())//new 2April
                ...
                arcs.insert(iter, std::pair<std::string, pair_with_edge *>(sig, pwe));
        }
    }
    ...
    while (arcs.size() > 0) {
        ...
        std::tr1::unordered_set<int> *D1 = domains_values.find(pwe->id1)->second;
        std::tr1::unordered_set<int> *D2 = domains_values.find(pwe->id2)->second;
        ...
        if (old_size1 != D1->size() && pNode->get_edges_size() > 1) {
            ...
            for (std::tr1::unordered_map<int, void *>::iterator iter1 = pNode->get_edges_iterator(); iter1 !=
                ...
                if (id2 == pwe->id2)
                    continue;
                std::string sig = get_sig(id1, id2, pwe->edge_label);
                ...
                if (arcs.find(sig) != arcs.end());//delete pwe;
                    ...
                    arcs.insert(std::pair<std::string, pair_with_edge *>(sig, pwe));
                }
            }
        }
 ...
 * @return
 */
bool GraMiCounter::refine(GraphX *graph, std::tr1::unordered_set<int> *D1, std::tr1::unordered_set<int> *D2, pair_with_edge *pwe,
                          int support) {
    ...
    for (std::tr1::unordered_set<int>::iterator iter = D1->begin(); iter != D1->end();) {
        ...
        for (std::tr1::unordered_map<int, void *>::iterator iter1 = node->get_edges_iterator(); iter1 !=
            ...
            if (edge->get_label() != edgeLabel)
                continue;
            std::tr1::unordered_set<int>::iterator iter_f = D2->find(edge->get_other_node()->get_id());
            if (iter_f != D2->end()) {
...
bool GraMiCounter::is_it_acyclic(GraphX &query) {
    ...
    while (visited.size() < query.get_num_of_nodes()) {
        ...
        //all edges!
        for (std::tr1::unordered_map<int, void *>::iterator iter = current_node->get_edges_iterator(); iter !=
 ...
 */
void GraMiCounter::set_domains_order(std::tr1::unordered_map<int, std::tr1::unordered_set<int> *> &domains_values, int *order,
                                     Pattern *pattern) {
    int invalid_col;
    ...
    std::tr1::unordered_set<int> added;
    int i = 0;
    ...
    for (; i < domains_values.size(); i++) {
        ...
        for (std::tr1::unordered_map<int, std::tr1::unordered_set<int> *>::iterator iter = domains_values.begin();
            ...
            std::tr1::unordered_set<int> *current_set = iter->second;
