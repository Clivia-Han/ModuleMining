...
bool check_current_status(GraphX *graph1, GraphX *graph2, std::vector<Set_Iterator *> &current_s, int *nodes_order,
    ...
    for (std::tr1::unordered_map<int, void *>::iterator iter = query_graph_node->get_edges_iterator(); iter !=
...
void insert_into_edge_freq(NodeX *src, int dest_node_id, std::string dest_node_label, std::string edge_label,
...
void GraphX::init(int id, int type) {
    ...
    this->type = type;
    if (type == 1) {
        std::cout << "IMPORTANT! current set of algorithms are not tested on directed graphs!";
        exit(0);
    }
    num_of_edges = 0;
    this->freq = -1;
...
GraphX::GraphX(GraphX *graph) {
    ...
    for (std::tr1::unordered_map<int, NodeX *>::const_iterator iter = graph->get_nodes_iterator(); iter !=
        ...
        for (std::tr1::unordered_map<int, void *>::iterator iter2 = old_node->get_edges_iterator(); iter2 !=
 ...
 */
NodeX *GraphX::add_node(int id, std::string label) {
    CL.clear();

    std::tr1::unordered_map<int, NodeX *>::iterator temp = nodes.find(id);
    if (temp != nodes.end())
        return temp->second;
...
bool GraphX::can_satisfy_node_labels(GraphX *other_g) {
    for (std::tr1::unordered_map<std::string, std::set<int> *>::iterator iter = other_g->nodes_by_label.begin();
         iter != other_g->nodes_by_label.end(); iter++) {
        std::string label = (*iter).first;
        int count = (*iter).second->size();
        if (count == 0) continue;

        std::tr1::unordered_map<std::string, std::set<int> *>::iterator this_iter = nodes_by_label.find(label);
 ...
 */
void GraphX::add_edge(NodeX *src_node, NodeX *dest_node, std::string edge_label) {
    CL.clear();
 ...
 */
void GraphX::add_edge(int src_id, int dest_id, std::string edge_label) {
    CL.clear();

    NodeX *src_node;
    NodeX *dest_node;

    std::tr1::unordered_map<int, NodeX *>::iterator iter = nodes.find(src_id);
    if (iter != nodes.end())
...
void GraphX::add_edge(int src_id, int dest_id, std::string edge_label, std::tr1::unordered_map<std::string, void *> &edge_to_freq,
    ...
    if (this->type == 0) {
        insert_into_edge_freq(dest_node, src_id, src_node->get_label(), edge_label, edge_to_freq, freq_edge_pairs,
                              false);
    }
}
 ...
 */
bool GraphX::load_from_file(std::string file_name, std::tr1::unordered_map<std::string, void *> &edge_to_freq) {
    CL.clear();

    std::cout << "Loading graph from file: " << file_name << std::endl;
 ...
 */
bool GraphX::load_from_string(std::string data, std::tr1::unordered_map<std::string, void *> &edge_to_freq) {
    CL.clear();
 ...
 */
bool GraphX::parse_data(std::istream &data, std::tr1::unordered_map<std::string, void *> &edge_to_freq,
                        std::map<std::string, std::map<int, std::set<int>>> &edge_pairs) {
    //read the first line
    char temp_ch;
    data >> temp_ch;
    ...
    while (!data.eof()) {
        ...
        } else if (ch == 'e')//to add edges
        {
            if (!first_edge_met) {
                ...
                if (freq > -1) {
                    for (std::tr1::unordered_map<std::string, std::set<int> *>::iterator iter = this->nodes_by_label.begin();
                         iter != nodes_by_label.end(); iter++) {
                        if (iter->second->size() < freq) {
                            //如果带某标签的节点在图中出现的次数小于阈值，也就意味着与带有该标签的节点相连的所有边都是不频繁的
                            std::tr1::unordered_map<std::string, std::set<int> *>::iterator iter2 = this->nodes_by_label.find(
                                    iter->first);
...
NodeX *GraphX::get_node_with_id(int node_id) {
    std::tr1::unordered_map<int, NodeX *>::iterator iter = nodes.find(node_id);
    if (iter == nodes.end())
        return NULL;
    else
        return iter->second;
...
std::set<int> *GraphX::get_nodes_by_label(std::string label) {
    std::tr1::unordered_map<std::string, std::set<int> *>::iterator iter = nodes_by_label.find(label);
    if (iter == nodes_by_label.end())
        return NULL;
    return iter->second;
...
std::tr1::unordered_map<int, NodeX *>::const_iterator GraphX::get_nodes_iterator() {
    return nodes.begin();
}

std::tr1::unordered_map<int, NodeX *>::const_iterator GraphX::get_nodes_end_iterator() {
    return nodes.end();
}
 ...
 */
std::string GraphX::get_edge_label(int src_node_id, int dest_node_id) {
    NodeX *srcNode;
    std::tr1::unordered_map<int, NodeX *>::iterator temp = nodes.find(src_node_id);
    if (temp == nodes.end())
        return "";
    srcNode = (*temp).second;
...
bool GraphX::is_connected() {
    ...
    std::map<int, NodeX *> visited;
    std::map<int, NodeX *> to_visit;
...
bool GraphX::is_the_same_with(GraphX *other_g) {
    ...
    //compare using CL
    std::string this_cl = this->get_canonical_label();
    std::string other_cl = other_g->get_canonical_label();
 ...
 */
void GraphX::is_isomorphic(GraphX *graph, std::vector<std::map<int, int> *> &results,
                           std::tr1::unordered_map<int, std::tr1::unordered_set<int> *> &domains_values,
    ...
    while (to_check.size() > 0) {
        ...
        for (std::tr1::unordered_map<int, void *>::iterator iter = curr_node->get_edges_iterator(); iter !=
    ...
    std::vector<Set_Iterator *> selected;
    std::tr1::unordered_set<int> selected_data_map;//only for fast check for existence
    std::tr1::unordered_map<int, int> selected_query_map;//map value is query node id, value is its order
    ...
    while (true) {
        ...
        if (postponed_nodes != nullptr) {
            if (num_iterations > max_iters) {
                ...
                {
                    std::vector<Set_Iterator *>::iterator iter = selected.begin();
        ...
        if (b) {
            ...
            if (selected.size() == this->get_num_of_nodes()) {
                ...
                int c = 0;
                for (std::vector<Set_Iterator *>::iterator i = selected.begin(); i != selected.end(); ++i, c++) {
                ...
                if (restricted_domain_id != -1) {
                    ...
                    for (std::vector<Set_Iterator *>::iterator iter = selected.begin(); iter != selected.end(); iter++) {
                        (*iter)->se->clear();
                        delete (*iter)->se;
                        delete (*iter);
                    }
            ...
            } else//no solution found yet!
            {
                ...
                std::vector<Set_Iterator *>::iterator temp_it = selected.begin();
                for (int i = 0; i < selected.size(); i++, temp_it++) {
                    // 找到子图上与tempN1相连的已选中的节点，找到该点对应在大图上的对应域的结点temp
                    if (temp_n1->is_it_connected_with_node_id(nodes_order[i])) {
                        temp = graph->get_node_with_id((*(*temp_it)->it));
                        break;
                    }
                }
                for (std::tr1::unordered_map<int, void *>::iterator iter = temp->get_edges_iterator(); iter !=
                    ...
                    if (selected_data_map.find(other_node_id) == selected_data_map.end()) {
                        ...
                        for (std::tr1::unordered_map<int, void *>::iterator iter1 = temp_n1->get_edges_iterator(); iter1 !=
 ...
 */
std::string GraphX::get_canonical_label() {
    if (CL.length() > 0) {
        return CL;
    }
...
std::ostream &operator<<(std::ostream &os, const GraphX &g) {
    os << "# t 1\n";
    //output the nodes
    for (std::tr1::unordered_map<int, NodeX *>::const_iterator ii = g.nodes.begin(); ii != g.nodes.end(); ++ii) {
    ...
    for (std::tr1::unordered_map<int, NodeX *>::const_iterator ii = g.nodes.begin(); ii != g.nodes.end(); ++ii) {
        ...
        for (std::tr1::unordered_map<int, void *>::iterator iter1 = node->get_edges_iterator(); iter1 !=
            ...
            if (node->get_id() < edge->get_other_node()->get_id())
                sig = int_to_string(edge->get_other_node()->get_id()) + "_" + edge->get_label() +
                      "_" +
                      int_to_string(
                              node->get_id());
            if (saved_edges.find(sig) == saved_edges.end()) {
                saved_edges.insert(sig);
                os << "e " << node->get_id() << " " << edge->get_other_node()->get_id() << " " << std::setprecision(10)
                   << edge->get_label() << "\n";
            }
        }
    }
...
//Destructor
GraphX::~GraphX() {
    for (std::tr1::unordered_map<int, NodeX *>::iterator ii = nodes.begin(); ii != nodes.end(); ++ii) {
        delete ii->second;
    }
    nodes.clear();
