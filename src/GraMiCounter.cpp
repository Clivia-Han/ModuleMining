...
int GraMiCounter::isFrequent(GraphX* graph, Pattern* pattern, int support, double approximate, map<int, set<int>>& domains_solutions)
{
	...
	for(int i=0;i<patternSize;i++)
	{
		...
		if(approximate!=-1)
		{
			...
			// 允许近似挖掘
			// 在原有的域中随机抽取size()*approximate数量的样本，重新定义currentDomain，以此判定该域是否满足支持度要求
		...
		for(tr1::unordered_set<int>::iterator iter = currentDomain->begin();iter!=currentDomain->end();++iter)
		{
			...
			counter++;
			// nodeID为当前正在处理的子图domainID对应的域中的大图结点
			int nodeID = (*iter);
			bool b = false;
				...
				else
				{
					...
					if(approximate==-1)
					{
						if(currentDomain->size()-counter+domains_solutions.find(domainID)->second.size()<support)
						{
							...
							// 在不允许近似挖掘的情况下，如果待检测结点加上有效结点的数量之和小于支持度，则对应的子图不频繁
							break;
						}
					}
					else
			...
			if(b)	//代表在currentDomain中找到有效的结点
			{
				if(approximate==-1)
				{
					...
					{
							// 遍历所有与domainID所对应的子图结点同构的大图上的结点，插入domain_solution中（相当于算法中将solution中的所有值在对应域标记）
				...
				for(vector<map<int, int>* >::iterator iter1 = result.begin();iter1!=result.end();iter1++)
				{
					...
					if(subTaskNum==-1)
					{
						for(map<int, int>::iterator iter2 = currentMapping->begin();iter2!=currentMapping->end();++iter2)
						{
							...
							int nID = iter2->second;
                            if (domains_solutions.find(dID) == domains_solutions.end()) {
                                cout << "dID not in domain_solutions!" << endl;
                                exit(0);
                            }
...
//		// targetAvg为在抽样数samplesBucket中满足频繁条件的期望值
...
{
	// 该方法要求对于较小的域D1，其中所有的结点都要满足：能够在D2中找到一个点与该点通过标签为edgeLabel的边相连，否则应该删除
	// 该D1中的点，于此同时D2中的点也应该满足在D1中能找到一点与之相连且边标签为edgeLabel，否则也删除
	// 当进行上述删除后，D1和D2中任意一个的域大小已经小于要求的支持度，那么返回false，否则返回true
	...
	for(tr1::unordered_set<int>::iterator iter = D1->begin();iter!=D1->end();)
	{
		...
		for(tr1::unordered_map<int, void*>::iterator iter1 = node->getEdgesIterator();iter1!=node->getEdgesEndIterator();++iter1)
		{
			...
			if(iter_f!=D2->end())
			{
				// 在D2中找到了一个结点，使得其通过一条标签为edgeLabel的边与D1中的结点相连
				deleteIt = false;
...
void GraMiCounter::setDomainsOrder(tr1::unordered_map<int, tr1::unordered_set<int>*>& domains_values, int* order, Pattern* pattern)
{
	...
	int i = 0;
	// 如果有预测为无效的列，将其排在前面（优先检测，寻求更早的剪枝）
	if(invalidCol>-1)
	{
		order[0] = invalidCol;
