...
public:
    bool init_unpre(const std::string& connect_table_file) {
        ...
        while (std::getline(infile, line)) {
            ...
            // erase space
            signal.erase(std::remove_if(signal.begin(), signal.end(), isspace),
                         signal.end());
            if (cell_type == "-") {
                continue;
            }
        ...
        for (auto sig_id : range(sig_cat.size())) {
            ...
            if (source_vec.size() > 1) {
                std::cerr << "Warning: signal with multi output: "
                          << signal_trie_.find_idx(sig_id) << "!\n";
            }
            for (const auto& source : source_vec) {
                for (const auto& target : target_vec) {
                    int edge_id = (int)edges.size();
    ...
    bool init(const std::string& pre_table) {
        ...
        while (std::getline(infile, line)) {
            ...
            if (!(cell_type.size() >= 3 && cell_type.substr(0, 3) == "sky")) {
                ...
                for (auto& view : raw_signal) {
                    ...
                    if (!np_signal_order_[node_id][cp_id].count(sig_id)) {
                        np_signal_order_[node_id][cp_id][sig_id] =
                                std::vector<int>();
                    }
                    np_signal_order_[node_id][cp_id][sig_id].push_back(ptr++);
                }
            }
        }
        // build edge_id_set
        for (auto sig_id : range(sig_cat.size())) {
            ...
            const auto& target_vec = sig_cat[sig_id].second;
            if (source_vec.size() > 1) {
                std::cerr << "Warning: module:" << module_name_
                          << " has signal with multi output: "
                          << signal_trie_.find_idx(sig_id) << "!\n";
            }
            // wire
            for (const auto& source : source_vec) {
    ...
    bool replace(int replace_node_id, System& sub) {
        const auto rep_full_name =
                full_name_trie_.find_idx(node_ref(replace_node_id).full_name_);
        if (attribute_trie_.find_idx(node_ref(replace_node_id).type_) !=
            sub.module_name_) {
        ...
        auto& nodes = storage_.nodes_;
        auto& edges = storage_.edges_;
        std::map<int, int> sub_node_map, sub_attr_map;
        ...
        for (auto& item : all_sub_attr_map) {
            sub_attr_map[item.first] =
                    attribute_trie_.insert(item.second).first;
        }
        // add inner nodes
        for (auto sub_node_id : sub.now_graph_.node_id_set) {
            const auto sub_node = sub.node_ref(sub_node_id);
            if (sub_node.type_ == _sub_id)
                continue;  // virtual node
            int new_node_id = (int)nodes.size();
            std::string hie_fn =
                    rep_full_name + "-" + all_sub_fn_map[sub_node.full_name_];
        ...
        std::map<int, std::map<int, std::vector<int>>> sub_sig_order_map;
        for (auto sub_edge_id : sub.now_graph_.edge_id_set) {
            ...
            if (sub.node_ref(sub_edge.source_).type_ == _sub_id) {
                if (!sub.np_signal_order_[sub_edge.source_][_sub_id].count(sub_edge.signal_name_)) {
                    std::cout << sub.module_name_ << '\n';
                    std::cout << sub_edge.source_ << ' ' << _sub_id << ' ' << sub.signal_trie_.find_idx(sub_edge.signal_name_) << '\n';
                    std::cout << 23333 << '\n';
                }
                int sig_rank = sub.np_signal_order_[sub_edge.source_][_sub_id]
                [sub_edge.signal_name_]
                        .front();
                if (!sub_sig_order_map[sub_edge.source_].count(sig_rank)) {
                    sub_sig_order_map[sub_edge.source_][sig_rank] =
                            std::vector<int>();
                }
                sub_sig_order_map[sub_edge.source_][sig_rank].push_back(
                        sub_edge_id);
                continue;
            } else if (sub.node_ref(sub_edge.target_).type_ == _sub_id) {
                if (!sub.np_signal_order_[sub_edge.target_][_sub_id].count(sub_edge.signal_name_)) {
                    std::cout << sub.module_name_ << '\n';
                    std::cout << sub_edge.target_ << ' ' << _sub_id << ' ' << sub.signal_trie_.find_idx(sub_edge.signal_name_) << '\n';
                    std::cout << 24444 << '\n';
                }
                int sig_rank = sub.np_signal_order_[sub_edge.target_][_sub_id]
                [sub_edge.signal_name_]
                        .front();
                if (!sub_sig_order_map[sub_edge.target_].count(sig_rank)) {
                    sub_sig_order_map[sub_edge.target_][sig_rank] =
                            std::vector<int>();
                }
                sub_sig_order_map[sub_edge.target_][sig_rank].push_back(
                        sub_edge_id);
                // int sig_rank =
                // sub.np_signal_order_[sub_edge.target_][_sub_id][sub_edge.signal_name_];
                // sub_sig_order_map[sub_edge.target_][sig_rank] = sub_edge_id;
                continue;
            }
            int new_edge_id = edges.size();
            std::string hie_sig =
                    rep_full_name + "-" + all_sub_sig_map[sub_edge.signal_name_];
            int new_sig_id = signal_trie_.insert(hie_sig).first;
            edges.push_back({new_sig_id,
                             sub_node_map[sub_edge.source_],
                             sub_attr_map[sub_edge.source_port_],
                             sub_node_map[sub_edge.target_],
                             sub_attr_map[sub_edge.target_port_],
                             {}});
            ...
            if (sub.np_signal_order_.count(sub_edge.source_)) {
                np_signal_order_[sub_node_map[sub_edge.source_]]
                [sub_attr_map[sub_edge.source_port_]]
                [new_sig_id] =
                        sub.np_signal_order_[sub_edge.source_]
                        [sub_edge.source_port_]
                        [sub_edge.signal_name_];
            }
            if (sub.np_signal_order_.count(sub_edge.target_)) {
                np_signal_order_[sub_node_map[sub_edge.target_]]
                [sub_attr_map[sub_edge.target_port_]]
                [new_sig_id] =
                        sub.np_signal_order_[sub_edge.target_]
                        [sub_edge.target_port_]
                        [sub_edge.signal_name_];
            }
        }
        ...
        for (auto edge_id : bef_edges) {
            ...
            if (edge.source_ == replace_node_id && edge.target_ == replace_node_id) {
                for (auto& rank1 : np_signal_order_[replace_node_id][edge.source_port_][edge.signal_name_]) {
                    for (auto sub_edge_id1 : sub_sig_order_map[port_map[edge.source_port_]][rank1]) {
                        const auto sub_edge1 = sub.edge_ref(sub_edge_id1);
                        for (auto& rank2 : np_signal_order_[replace_node_id][edge.target_port_][edge.signal_name_]) {
                            for (auto sub_edge_id2 : sub_sig_order_map[port_map[edge.target_port_]][rank2]) {
                                const auto sub_edge2 = sub.edge_ref(sub_edge_id2);
                                int new_edge_id = (int)edges.size();
                                edges.push_back(
                                        {edge.signal_name_, sub_node_map[sub_edge1.source_],
                                         sub_attr_map[sub_edge1.source_port_], sub_node_map[sub_edge2.target_],
                                         sub_attr_map[sub_edge2.target_port_], edge.attributes_});
                                aft_edges.push_back(new_edge_id);
                            }
                        }
                    }
                }
            } else if (edge.source_ == replace_node_id) {
                for (auto& rank :
                        np_signal_order_[replace_node_id][edge.source_port_]
                        [edge.signal_name_]) {
                    int sub_node_id = port_map[edge.source_port_];
                    for (auto sub_edge_id :
                            sub_sig_order_map[sub_node_id][rank]) {
                        const auto sub_edge = sub.edge_ref(sub_edge_id);
                        int new_edge_id = (int)edges.size();
                        edges.push_back(
                                {edge.signal_name_, sub_node_map[sub_edge.source_],
                                 sub_attr_map[sub_edge.source_port_], edge.target_,
                                 edge.target_port_, edge.attributes_});
                        aft_edges.push_back(new_edge_id);
                    }
                }
            } else if (edge.target_ == replace_node_id) {
                for (auto& rank :
                        np_signal_order_[replace_node_id][edge.target_port_]
                        [edge.signal_name_]) {
                    ...
                    for (auto sub_edge_id :
                        ...
                        const auto sub_edge = sub.edge_ref(sub_edge_id);
                        int new_edge_id = (int)edges.size();
                        edges.push_back({edge.signal_name_, edge.source_,
                                         edge.source_port_,
                                         sub_node_map[sub_edge.target_],
                                         sub_attr_map[sub_edge.target_port_],
                                         edge.attributes_});
                ...
                // int sig_rank =
                // np_signal_order_[replace_node_id][edge.target_port_][edge.signal_name_];
                // int sub_node_id = port_map[edge.target_port_];
                // auto add_inner_edge = [&](int rank) {
                //     int sub_edge_id = sub_sig_order_map[sub_node_id][rank];
                //     const auto sub_edge = sub.edge_ref(sub_edge_id);
                //     std::string hie_sig = rep_full_name + "-" +
                //     all_sub_sig_map[sub_edge.signal_name_]; int new_sig_id =
                //     signal_trie_.insert(hie_sig).first; int new_edge_id =
                //     (int) edges.size(); edges.push_back({new_sig_id,
                //     edge.source_, edge.source_port_,
                //                     sub_node_map[sub_edge.target_],
                //                     sub_attr_map[sub_edge.target_port_],
                //                     edge.attributes_});
                //     aft_edges.push_back(new_edge_id);
                // };
                // if
                // (np_signal_order_[replace_node_id][edge.target_port_].size()
                // != sub_sig_order_map[sub_node_id].size()) {
                //     for (auto i :
                //     range(sub_sig_order_map[sub_node_id].size())) {
                //         add_inner_edge(i);
                //     }
                // } else  {
        ...
        logs_.data_.push_back(
                {REPLACE, {bef_nodes, bef_edges}, {aft_nodes, aft_edges}});
        return true;
    }

    // shink sub to node
    // haven't build sub's primary input & output
    bool shink(std::set<int> sub,
               const std::string shink_cell_name,
               const std::string shink_cell_type) {
        ...
        for (auto edge_id : now_graph_.edge_id_set) {
            const auto edge = edge_ref(edge_id);
            bool source_in = sub.count(edge.source_),
                    target_in = sub.count(edge.target_);
            ...
            if (source_in) {
                int new_edge_id = (int)edges.size();
                edges.push_back({edge.signal_name_, shink_node_id,
                                 edge.source_port_, edge.target_,
                                 edge.target_port_, edge.attributes_});
                aft_edges.push_back(new_edge_id);
                // TODO: port merge rule unknow
                // int ptr = (int)
                // np_signal_order_[shink_node_id][edge.source_port_].size();
                // np_signal_order_[shink_node_id][edge.source_port_][edge.signal_name_]
                // = ptr;
            } else if (target_in) {
                int new_edge_id = (int)edges.size();
                edges.push_back({edge.signal_name_, node_map[edge.source_],
                                 edge.source_port_, shink_node_id,
                                 edge.target_port_, edge.attributes_});
                aft_edges.push_back(new_edge_id);
                // TODO: port merge rule unknow
                // int ptr = (int)
                // np_signal_order_[shink_node_id][edge.target_port_].size();
                // np_signal_order_[shink_node_id][edge.target_port_][edge.signal_name_]
                // = ptr;
            }
        }
        update_graph({bef_nodes, bef_edges}, {aft_nodes, aft_edges});
        logs_.data_.push_back(
                {SHINK, {bef_nodes, bef_edges}, {aft_nodes, aft_edges}});
        return true;
    }

    void update_graph(
            const std::pair<std::vector<int>, std::vector<int>>& bef,
    ...
    bool roll_back(bool canceable = true, int sz = 1) {
        ...
        if (logs_.data_.size() < sz) {
            std::cerr << "Error: logs_size=" << logs_.data_.size()
                      << " < roll_back_size!\n";
            return false;
        }
    ...
    bool write_networkX(const std::string& path) {
        /*
            Tips:
                1. The read interface implemented by Networkx inside
           'networkx/readwrite/graphml.py' does not support mixed graphs
           (directed and unidirected edge_id_set together), hyperedges,
                   **nested graphs**( Hierarchical graph structure ), or
           **ports**( Treated as parts of node_id_set ).
                       - This is the description written in the comments of the
           file
           ...
           'boost/property_map/dynamic_properties' later
                    - make the mapping from variables to values more flexible
                      i.e. aotumatically get the members of the data structs(
           Node, Edge ) instead of fixing
                2. Refer [Write/read vector of item in xml using
           boost::write_graphml](
        ...
        */
        ...
        std::map<std::string, std::pair<std::string, bool>>
                key_name_type_isnode = {{"full_name", {"string", true}},
        ...
        for (int node_id : now_node_id_set) {
            ...
            out << "      <data key=\"" << key_ids["full_name"] << "\">"
                << full_name_mapping[nod.full_name_] << "</data>\n";
            const auto& node_attrs =
                    node_attributes(node_id);  // cell_type has been included
            if (!node_attrs.empty()) {
        ...
        for (auto edge_id : now_edge_id_set) {
            ...
            out << "    <edge id=\"e" << edge_id << "\" source=\"n"
                << edg.source_ << "\" target=\"n" << edg.target_ << "\">\n";
            ...
            out << "      <data key=\"" << key_ids["signal"] << "\">"
                << signal_mapping[edg.signal_name_] << "</data>\n";
            out << "    </edge>\n";
        }
        out << "  </graph>\n"
            << "</graphml>\n";
    ...
    // private:
    Node& node_ref(int node_id) { return storage_.nodes_[node_id]; }

    Edge& edge_ref(int edge_id) { return storage_.edges_[edge_id]; }
